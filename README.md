...
| 阶段四 | 🔄 增量 A | 单层 `QLinearConv`（`qlinearconv_debug.onnx`）Python 桥接 → MLIR → 二进制；`instructions/uop` 字节级 + FSIM 对齐黄金（`scripts/run_onnx_qconv.sh`） |

尚未实现（后续增量）：onnx-mlir C++ 前端、partial row block、两层 Conv + fsim_nn、MaxPool/Relu/Mul、CPU 算子、LeNet-5 整网。
